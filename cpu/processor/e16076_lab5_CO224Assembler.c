...
This simple assembler assumes an ISA containing the following instructions: loadi, mov, add, sub, and, or, j, beq, load, store, bne, sll, srl, sra, ror. 
All instructions are encoded into 32-bit words based on the following format:

Bits 31-24 : OP-CODE		: Given as one of (loadi, mov, add, sub, and, or, j, beq, load, store, bne, sll, srl, sra, ror)
...
Generated output file	: <assembly_file_name>.machine
*/
...
int main( int argc, char *argv[] )
{
	...
	char *op_or 	= "00000101";
	char *op_j	= "00000110";//jump
	char *op_beq	= "00000111";//branch if equal
	char *op_load 	= "00001000";
	char *op_store 	= "00001001";
        //following are to make new commands for part 5
        char *op_bne 	= "00001010";// branch not equal
	char *op_sll 	= "00001011";//logical shift left
	char *op_srl	= "00001100";//logical shift right
	char *op_sra	= "00001101";//arithmetic shift right
	char *op_ror 	= "00001110";//rotate right
	//char *op_store 	= "00001001";
 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*******************************************************************/
		...
		while(in_token!=NULL)
		{
			...
			// Encoding the op-code----------have to change this
